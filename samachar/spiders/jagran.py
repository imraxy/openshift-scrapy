    ...
    def parse(self, response):
        ...
        for news in news:
            ...
            item['title'] = news.xpath(
                "a/div[@class='protxt fr']/div[@class='h3']/text()").extract_first()
            item['url'] = 'https://www.jagran.com' + \
                news.xpath("a/@href").extract_first()
    ...
    def parse_detail_page(self, response):
        ...
        if detailPageSelector.xpath("//figure[@class='bodySummery']/figcaption/text()").extract_first():
            item['shortdesc'] = detailPageSelector.xpath(
                "//figure[@class='bodySummery']/figcaption/text()").extract_first()
            item['description'] = detailPageSelector.xpath(
                "//div[@class='articleBody']/p[2]").extract_first()
        else:
            item['shortdesc'] = detailPageSelector.xpath(
                "//div[@id='topHeading']/h1/text()").extract_first()
            item['description'] = detailPageSelector.xpath(
                "//div[@class='articleBody']/p/text()").extract_first()

        item['img_title'] = detailPageSelector.xpath(
            "//ul[@id='output']/li/a/@title").extract_first()

        if detailPageSelector.xpath("//img[@id='jagran_image_id']/@src").extract_first():
            item['img_urls'] = detailPageSelector.xpath(
                "//img[@id='jagran_image_id']/@src").extract_first()
        else:
            item['img_urls'] = detailPageSelector.xpath(
                "//div[@class='lt-image']/img/@src").extract_first()
